...
import { reportError, installGlobalErrorHandlers } from "@/lib/observability/reporter";

// Wire RN's top-level error / rejection hooks into the reporter so unhandled
// failures land in our log channel instead of dying silently in production.
// Module-level call: runs once when JS first loads, before any component mounts.
...
// Minimum splash duration — keeps branding visible briefly even when the
// app is ready to render sooner. Pure UX request, not a technical need.
...
function AuthGuard() {
  ...
  useEffect(() => {
    ...
    // PIN setup gate: with a token but no PIN, force the setup flow before
    // letting the user reach the tabs.
...
// DB init state. Render nothing while open()+migrations are in flight (splash
// stays up); proceed once `ready`. Migration failures are non-fatal — they
// get reported and the app launches; cache reads degrade gracefully without
// SQLite while subsequent open() retries inside fetchers recover.
type DbInitState = { kind: "loading" } | { kind: "ready" };
